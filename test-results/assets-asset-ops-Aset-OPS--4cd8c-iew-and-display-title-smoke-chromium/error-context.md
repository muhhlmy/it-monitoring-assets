...
- Name: assets\asset-ops.spec.js >> Aset OPS Management Suite >> Should open Aset OPS view and display title @smoke
- Location: e2e\tests\assets\asset-ops.spec.js:4:3
...
Locator: getByRole('heading', { name: /aset ops/i }).last()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: /aset ops/i }).last()
...
```ts
  ...
  2  | 
  3  | test.describe('Aset OPS Management Suite', () => {
  4  |   test('Should open Aset OPS view and display title @smoke', async ({ superAdminPage }) => {
  5  |     const page = superAdminPage
  6  |     await page.goto('/assets-ops', { waitUntil: 'domcontentloaded' })
  7  | 
> 8  |     await expect(page.getByRole('heading', { name: /aset ops/i }).last()).toBeVisible({ timeout: 10000 })
     |                                                                           ^ Error: expect(locator).toBeVisible() failed
  9  |   })
  10 | })
  11 | 
